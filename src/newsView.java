	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			try {
				...
				String dateOfNews="null";
				JSONArray arrayObj=new JSONArray();
				Statement statement = connection.createStatement();
				try{
					 dateOfNews=request.getParameter("date").toString();
					 
				        dateOfNews = dateOfNews.replace('/', '-');
				        
				        
				        String month=dateOfNews.substring(0, 2);
				        String date=dateOfNews.substring(3, 5);
				        String year=dateOfNews.substring(6, 10);
				        dateOfNews=year+"-"+month+"-"+date;
				}catch(Exception e){}
				
				
				//System.out.println(query);
				 String query="";
				if(dateOfNews.equalsIgnoreCase("Enter"))
				{
					query = "SELECT * FROM news";
				}
				else
				{
					query = "SELECT * FROM news where date='"+dateOfNews+"'";
				}
