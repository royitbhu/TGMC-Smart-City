	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			try {
				...
				try{
					 movie_name=request.getParameter("movie_name").toString();
				}catch(Exception e){}
				System.out.println(movie_name);
				String mall_name="Select a Theatre";
				try{
					 mall_name=request.getParameter("theatre_name").toString();
				}catch(Exception e){}
				System.out.println(mall_name);
				//System.out.println(query);
				String query="";
				if(movie_name.equalsIgnoreCase("null") && mall_name.equalsIgnoreCase("Select a Theatre"))
					query = "SELECT * FROM movies";
				else{
					if(!movie_name.equalsIgnoreCase("null") && !mall_name.equalsIgnoreCase("Select a Theatre"))
					{
						query = "SELECT * FROM movies where movie_name='"+movie_name+"' and mall_name='"+mall_name+"'";
					}
					else{
						if(!movie_name.equalsIgnoreCase("null"))
						{
							query = "SELECT * FROM movies where movie_name='"+movie_name+"'";
						}
						if(!mall_name.equalsIgnoreCase("Select a Theatre"))
						{
							query = "SELECT * FROM movies where mall_name='"+mall_name+"'";
						}
					}
				}
				...
				while(resultSet.next()){
					count++;
						movie_name = resultSet.getString(2);
						mall_name = resultSet.getString(3);
