        ...
        var submit = fs.addButton({
			...
			if(fs.getForm().isValid()){					
				fs.getForm().submit({
					...
					waitMsg:'Saving Data...',
	            		success : function(){
								Ext.Msg.alert('Status','Tax Update Successful',function(btn,text){
									//if(btn=='ok'){
...
/*
						...
						Ext.Msg.alert('TAX Update Failed','Please fill in the essential details') ;
				}
            }
        });
        // simple button add
...
*/
//Ext.apply(fs.form.baseParams,{tax_name:'tax_name'});

		var thetax_name = Ext.getCmp('tax_name');
          Ext.Ajax.request({
        	  method: 'GET',
        	  params:{tax_name:thetax_name.value},
				url:'../loadTaxes',
				waitMsg: 'Loading...',
            	...
            	var resp = Ext.decode(response.responseText);
	             	//store.removeAll();
	             	store.loadData(resp.reply);	
	             	Ext.getCmp('tax_name').setValue(store.getAt(0).data.tax_name);
	             	Ext.getCmp('paid_by').setValue(store.getAt(0).data.paid_by);
	             	Ext.getCmp('details').setValue(store.getAt(0).data.details);
	             	submit.enable();
	             	/*
                	var resp = Ext.decode(response.responseText);
                	var i =0 ;
                	document.write("<table border 0><tr><td>Heading</td><td>Details</td><td>Source</td><td>Date Posted</td></tr>");
					for(i=0;i<resp.size;i++)
					{
						
						document.write("<tr><td>"+resp.reply[i].fname+"</td>");
						document.write("<td>"+resp.reply[i].lname+"</td>");
						document.write("<td>"+resp.reply[i].gender+"</td>");
						document.write("<td>"+resp.reply[i].country+"</td></tr>");
